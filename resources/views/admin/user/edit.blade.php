        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <div class="table-responsive">
                    <form class="user-form" action="{{ route('user.update', $user) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="password" class="form-control @error('password') is-invalid @enderror"
                                    id="inputPassword" name="password" value="{{ old('password', $user->password) }}"
                        ...
                        @if (Auth::user()->role == 'admin')
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('role') is-invalid @enderror" id="inputrole"
                                    ...
                                    <option value="Admin Fakultas" {{ old('role', $user->role) == 'Admin Fakultas' ? 'selected' : '' }}>
                                        Admin Fakultas</option>
                                    <option value="Admin Departemen"
                                        {{ old('role', $user->role) == 'Admin Departemen' ? 'selected' : '' }}>Admin Departemen</option>
                                </select>
                                @error('role')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        @elseif(Auth::user()->role == 'Admin Fakultas')
                        <div class="mb-3">
                            <label for="inputRole" class="form-label">Sebagai</label>
                            <select class="form-select @error('role') is-invalid @enderror" id="inputrole"
                                name="role" onchange="toggleRoleFields()">
                            ...
                            <select class="form-select @error('fakultas_id') is-invalid @enderror" id="inputfakultas_id"
                                ...
                                @foreach ($fakultases as $fakultas)
                                    <option value="{{ $fakultas->id }}"
                                        {{ old('fakultas_id', $user->fakultas_id) == $fakultas->id ? 'selected' : '' }}>
                                        {{ $fakultas->nama }}
                                    </option>
                                @endforeach
                                @else
