        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <div class="table-responsive">
                    <form class="fakultas-form" action="{{ route('fakultas.update', $fakulta) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('kode') is-invalid @enderror" id="inputkode"
                                name="kode" value="{{ old('kode', $fakulta->kode) }}" placeholder="FIP">
                            @error('kode')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama') is-invalid @enderror" id="inputNama"
                                name="nama" value="{{ old('nama', $fakulta->nama) }}" placeholder="Nama">
                            @error('nama')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="inputdeskripsi" name="deskripsi"
                                placeholder="Deskripsi tentang fakultas">{{ old('deskripsi', $fakulta->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
